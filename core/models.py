...
class StreamProfile(models.Model):
    ...
    def build_command(self, stream_url, user_agent, proxy=None):
        ...
        # Add HTTP proxy parameter for FFmpeg if proxy is provided
        if proxy and proxy.strip() and self.command == "ffmpeg":
            # Insert proxy parameter before the input URL
            # Find the position of -i parameter
            try:
                i_index = cmd.index('-i')
