...
@api_view(['POST'])
def clear_engine_cache(request, account_id=None):
    """
    ...
    POST /api/m3u/clear-engine-cache/<account_id>/
    (Gets portal_url from account)
    
    Returns:
        200: Cache cleared successfully
        400: Invalid request
    """
    # If account_id is in URL, get portal_url from account
    if account_id:
        ...
        try:
            ...
            if not hasattr(request, 'data'):
                request._full_data = {}
            request.data['portal_url'] = portal_url
