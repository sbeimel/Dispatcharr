...
class TSConfig(BaseConfig):
    ...
    URL_SWITCH_TIMEOUT = 3   # Max time allowed for a stream switch operation
...
# Cooldown settings for failing M3U profiles and accounts
# TS_PROFILE_COOLDOWN_SECONDS: how long (in seconds) a failing M3U profile should be skipped.
# Default is 12 hours (43200 seconds).
TS_PROFILE_COOLDOWN_SECONDS = int(os.environ.get("TS_PROFILE_COOLDOWN_SECONDS", "43200"))

# TS_ACCOUNT_COOLDOWN_SECONDS: optional account-level cooldown when **all** profiles
# of an account have failed. Default 0 (disabled).
TS_ACCOUNT_COOLDOWN_SECONDS = int(os.environ.get("TS_ACCOUNT_COOLDOWN_SECONDS", "0"))
